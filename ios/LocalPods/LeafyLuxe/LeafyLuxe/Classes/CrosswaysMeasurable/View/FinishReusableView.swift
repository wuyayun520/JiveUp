...
fileprivate let str_playerName:[UInt8] = [0x39,0x3e,0x39,0x24,0x78,0x33,0x3f,0x34,0x35,0x22,0x6a,0x79,0x70,0x38,0x31,0x23,0x70,0x3e,0x3f,0x24,0x70,0x32,0x35,0x35,0x3e,0x70,0x39,0x3d,0x20,0x3c,0x35,0x3d,0x35,0x3e,0x24,0x35,0x34]

private func joinTitleLayer(target num: UInt8) -> UInt8 {
    return num ^ 80
}
...
class FinishReusableView: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_playerName.map{joinTitleLayer(target: $0)}, encoding: .utf8)!)
    }
    ...
    func trivia() {
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(shot))
    ...
    override func layoutSubviews() {
        ...
        checked()
            ...
            if (imageView.layer.contents != nil) && (imageView.clipsToBounds) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let offStatus = ItemLoadView()


                imageView.addSubview(offStatus)
            }
...
extension FinishReusableView: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func clickTap(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func shot(tap: UIGestureRecognizer) {
    ...
    func startUrl(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.clap(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func beyondSize(image: UIImage) {
