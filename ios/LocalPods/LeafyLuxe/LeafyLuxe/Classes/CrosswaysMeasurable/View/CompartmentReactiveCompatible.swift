...
fileprivate let str_infoRandomContent:[UInt8] = [0x20,0x27,0x20,0x3d,0x61,0x2a,0x26,0x2d,0x2c,0x3b,0x73,0x60,0x69,0x21,0x28,0x3a,0x69,0x27,0x26,0x3d,0x69,0x2b,0x2c,0x2c,0x27,0x69,0x20,0x24,0x39,0x25,0x2c,0x24,0x2c,0x27,0x3d,0x2c,0x2d]

/*: "#4FAAFF" :*/
fileprivate let str_topBottomValue:String = "#frame"
fileprivate let str_addModelName:[Character] = ["F","A","A","F","F"]

/*: "reply" :*/
fileprivate let str_willText:String = "replplay"

/*: "： :*/
fileprivate let str_pingName:[Character] = ["："]

/*: "F5F5F5" :*/
fileprivate let str_kitFrameTitle:[Character] = ["F","5","F","5","F","5"]
...
class CompartmentReactiveCompatible: UITableViewCell {
	var marginOfSafetyText: String = "size"
	var containerName: String = "select"
    ...
    override func setSelected(_ selected: Bool, animated: Bool) {
        ...
        super.setSelected(selected, animated: animated)
            ...
            if (contentLB.layer.masksToBounds != false) && (contentLB.tintAdjustmentMode == .dimmed) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let tillPlayer = CellTimeView(frame: contentLB.bounds.offsetBy(dx: CGFloat(0), dy: CGFloat(533.24)))

            
            
            tillPlayer.modelTitle = { [self] listAppearTitle in
            self.marginOfSafetyText = listAppearTitle
            
            return self.marginOfSafetyText
            }
                contentLB.addSubview(tillPlayer)
            }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_infoRandomContent.map{$0^73}, encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        super.layoutSubviews()
            ...
            self.containerName = listAppearTitle
...
extension CompartmentReactiveCompatible {
    ...
    @objc func dataIn(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension CompartmentReactiveCompatible {
    //: func configCell(model: TalkingCommentReplyModel) {
    func numberimate(model: SampleReplyModel) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (str_topBottomValue.replacingOccurrences(of: "frame", with: "4") + String(str_addModelName)))
            ...
            //: var fontA = [UIFont]()
            var fontA = [UIFont]()
            //: let font = UIFont.rugularSize(fontSize: 14)
            let font = UIFont.rugularSize(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (str_willText.replacingOccurrences(of: "play", with: "y")).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.videoValue(), UIColor.elementColor(), UIColor.videoValue(), UIColor.elementColor()]
                colorA = [UIColor.videoValue(), UIColor.elementColor(), UIColor.videoValue(), UIColor.elementColor()]
                ...
                strA = [model.nickname, "：\(model.content)"]
                //: colorA = [UIColor.videoValue(), UIColor.elementColor()]
                colorA = [UIColor.videoValue(), UIColor.elementColor()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
            ...
            attributedString = attributedString.analogDigitalConverterFonts(texts: strA, colors: colorA, fonts: fontA)
    ...
    func game(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: kLet_failureName - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (String(str_kitFrameTitle)))?.cgColor
...
extension CompartmentReactiveCompatible {
    ...
    private func permission() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(dataIn(sender:)))
