...
/*: "%@%@" :*/
fileprivate let str_packName:String = "%@%@"

/*: "/TXCache" :*/
fileprivate let str_snapValue:String = "/TXCacheengine model user true app"
...
// __CLOSE_PRINT__
//
//  FinishErasePlayerManager.swift
//  AbroadTalking
//
...
protocol CostRowReactiveCompatible: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func infoAcrossStatus(player: FinishErasePlayerManager, status: AcrossBorderPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func contextOfUse(player: FinishErasePlayerManager, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func sizePop(player: FinishErasePlayerManager, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class FinishErasePlayerManager: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: CostRowReactiveCompatible?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: AcrossBorderPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = FinishErasePlayerManager()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(str_snapValue.prefix(8)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(scurry), userInfo: nil, repeats: true)
...
extension FinishErasePlayerManager {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func nose(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: kLet_nameStreamLimitData)
    ...
    @objc func scurry() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.sizePop(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func sumeraction(renderMode: CellViewRenderMode) {
    ...
    func bigness(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isTalking == true {
        if TitleSocketManager.shared.isTalking == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func clickPause() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.infoAcrossStatus(player: self, status: self.playStatus!)
        }
    }
...
extension FinishErasePlayerManager: TXVodPlayListener {
    ...
    func onPlayEvent(_ player: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.contextOfUse(player: self, duration: Int(duration), currentTime: Int(currentTime))
