...
/*: "id" :*/
fileprivate let str_roomTitle:String = "color"
...
struct LightMeasurable: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
    ...
    var pic = [VendorPhotoModel]()
    //: var momentType: String?
    var momentType: String?
    //: var videoInfo: TalkingMomentVideoModel?
    var videoInfo: ViewMTheoryModelType?
...
struct VendorPhotoModel: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
struct ViewMTheoryModelType: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: DisappearReplaceableCollection?
}

///
//: enum MomentVideoStatus: String {
enum DisappearReplaceableCollection: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension LightMeasurable {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func addPost(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            //: textHeight = Int(TalkingMomentTextSizeView.caculateSizeWithText(text: content!, size: CGSize.init(width: ScreenWidth-54, height: CGFloat(MAXFLOAT)), font: UIFont.magnitudeimateBy(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber==0 ? 20:0))
            textHeight = Int(DismissThen.sizeTo(text: content!, size: CGSize(width: kLet_failureName - 54, height: CGFloat(MAXFLOAT)), font: UIFont.magnitudeimateBy(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == VarietySendable.Photo.rawValue || momentType == VarietySendable.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
