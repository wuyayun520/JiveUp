...
fileprivate let str_popTitle:[UInt8] = [0x47,0x40,0x47,0x5a,0x6,0x4d,0x41,0x4a,0x4b,0x5c,0x14,0x7,0xe,0x46,0x4f,0x5d,0xe,0x40,0x41,0x5a,0xe,0x4c,0x4b,0x4b,0x40,0xe,0x47,0x43,0x5e,0x42,0x4b,0x43,0x4b,0x40,0x5a,0x4b,0x4a]

private func startTransform(label num: UInt8) -> UInt8 {
    return num ^ 46
}

/*: "FinishReusableView" :*/
fileprivate let str_whiteData:String = "tool color in manager inTalkin"
fileprivate let str_neverHiddenData:String = "connection in varntPh"
fileprivate let str_modelCrushTitle:[Character] = ["r","e","v","i","e","w","C","e","l","l"]

/*: "Save successfully" :*/
fileprivate let str_sexValue:[Character] = ["S","a","v","e"," ","s","u","c","c","e","s","s","f","u","l","l","y"]
...
class ConversationViewDelegate: ShareRecognizerDelegate {
	var actionQuantity: Int = 29
	var tableCount: Double = -69.1
	var dirtArray: [AnyHashable] = []
	var vocalizationTotal: Int = 81
	var sendInterval: Double = -8.7
    ...
    init(momentModel: LightMeasurable, index: Int, type: ModelPhotoType) {
        ...
        if PropertyLineThen.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(kLet_failureName), y: 0)
    ...
    init(images: [UIImage], index: Int, type: ModelPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_popTitle.map{startTransform(label: $0)}, encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        hide(modle: currenModel)
//        if myType == .normal {
//            setDownloadView()
//        }
    
            if (collecView.inputViewController != nil) && (collecView.convert(CGRect(x: 0, y: CGFloat(0), width: 0, height: 0), from: collecView.superview).origin.y == 84.19) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let saveFrom = InfoOfView(frame: collecView.frame.offsetBy(dx: CGFloat(67), dy: CGFloat(542.28)))

            
            saveFrom.nameCompleteMagnitude = { [self] sumCount in
            self.actionQuantity = sumCount
            
            var saveFrom = pageIndex
            saveFrom &+= 1
            if saveFrom > self.actionQuantity {
                self.actionQuantity = saveFrom
            }
            
            return self.actionQuantity
            }
            saveFrom.viewEnableTotal = { [self] collectionMagnitude in
            self.tableCount = collectionMagnitude
            ...
            self.dirtArray = imagePlayArray
            
            guard var value = self.dirtArray as? [String] else {
                return nil
            }
            return value
            }
                collecView.addSubview(saveFrom)
            }
    ...
    override func observerData() {
        ...
        callTitleLabel()
            ...
            if (self.bottomView.inputViewController != nil) && (self.bottomView.convert(CGRect(x: 0, y: CGFloat(0), width: 0, height: 0), from: self.bottomView.superview).origin.y == 84.19) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let saveFrom = InfoOfView(frame: self.bottomView.frame.offsetBy(dx: CGFloat(67), dy: CGFloat(542.28)))

            
            saveFrom.nameCompleteMagnitude = { [self] sumCount in
            self.vocalizationTotal = sumCount
            
            var saveFrom = pageIndex
            saveFrom &+= 1
            if saveFrom > self.vocalizationTotal {
                self.vocalizationTotal = saveFrom
            }
            
            return self.vocalizationTotal
            }
            saveFrom.viewEnableTotal = { [self] collectionMagnitude in
            self.sendInterval = collectionMagnitude
            
                self.sendInterval += 1
                if self.sendInterval >= 0 {
                    self.sendInterval = self.sendInterval - 1
                }
            return self.sendInterval
            }
            saveFrom.endArray = { [self] imagePlayArray in
            self.willArray = imagePlayArray
            ...
            return value
                ...
                self.bottomView.addSubview(saveFrom)
            }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: kLet_failureName, height: kLet_appEventDomainName)
        ...
        view.frame = CGRect(x: 0, y: 0, width: kLet_failureName, height: kLet_appEventDomainName)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(kLet_failureName), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(kLet_failureName), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(kLet_failureName), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: "TalkingMomentPhotosPreviewCell")
        view.register(FinishReusableView.self, forCellWithReuseIdentifier: (String(str_whiteData.suffix(6)) + "gMome" + String(str_neverHiddenData.suffix(4)) + "otosP" + String(str_modelCrushTitle)))
...
//    lazy var downloadBtn: OfTalkingButton = {
//        let btn = OfTalkingButton.init()
...
extension ConversationViewDelegate {
...
//        AccountingDataReactiveCompatible.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: VendorPhotoModel = self.currenModel.pic[self.pageIndex]
...
//    }
    ...
    func magnitudelessness(image: UIImage) {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.playerMsg(showMsg: (String(str_sexValue)).localized)
                }
                //: } else {
            } else {
...
extension ConversationViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func ventureCapital() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func hidebottomObserver() {
        //: if  currenModel.sex==SubLabelReactiveCompatible.share.loginUserMode.sex {
        if currenModel.sex == SubLabelReactiveCompatible.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: FinishReusableView = collectionView.dequeueReusableCell(withReuseIdentifier: (String(str_whiteData.suffix(6)) + "gMome" + String(str_neverHiddenData.suffix(4)) + "otosP" + String(str_modelCrushTitle)), for: indexPath) as! FinishReusableView
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: VendorPhotoModel = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.startUrl(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.beyondSize(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.beyondSize(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: FinishReusableView = cell as! FinishReusableView
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / kLet_failureName)
        //: if myType == .normal {
        if myType == .normal {
