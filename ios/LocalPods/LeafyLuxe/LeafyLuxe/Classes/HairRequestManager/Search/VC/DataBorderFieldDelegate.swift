...
/*: "Search" :*/
fileprivate let str_withRoomName:String = "color"
fileprivate let str_toGiftName:[Character] = ["e","a","r","c","h"]

/*: "#F5F5F5" :*/
fileprivate let str_hiddenText:[Character] = ["#","F"]
fileprivate let str_managerValue:[Character] = ["5","F","5","F","5"]

/*: "Search by UID" :*/
fileprivate let str_resumeMidValue:String = "Search gift on distance self type"
fileprivate let str_leadingText:String = "text loadby UID"

/*: "icon_search_sea" :*/
fileprivate let str_makeValue:String = "icon_self color equal size any"
fileprivate let str_assetName:String = "ch_sealeading self block shadow"

/*: "Can’t find~" :*/
fileprivate let str_bubbleSizeName:[Character] = ["C","a","n","’","t"," ","f","i","n","d","~"]

/*: "icon_kong_kong_default" :*/
fileprivate let str_actualValue:[Character] = ["i","c","o","n","_","k","o","n","g","_","k","o","n","g","_","d","e"]
fileprivate let str_labelValue:[Character] = ["f","a","u","l","t"]

/*: "Please enter the query UID" :*/
fileprivate let str_rawText:[Character] = ["P","l","e","a","s","e"," ","e","n","t","e","r"," ","t","h","e"]
fileprivate let str_shareData:String = " quertrack self"
...
class DataBorderFieldDelegate: EqualVideoReactiveCompatible {
	var currentName: String = "pop"
    ...
    override func viewDidLoad() {
        ...
        stopVideo()
            ...
            if (searchTF.superview != nil && !searchTF.isDescendant(of: searchTF.superview!)) && (searchTF.layer.contentsRect.size.height != 1) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let searchLet = PlotView(frame: searchTF.frame.offsetBy(dx: CGFloat(60), dy: CGFloat(285.91)))
            searchLet.faceCount = reqManager.recommendListPage


            
            searchLet.addText = { [self] evanesceContent in
            self.currentName = evanesceContent
            
            return self.currentName
            }
            searchLet.drawContentTowardArray = { [self] createArray in
            self.dataArray = createArray
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((str_withRoomName.replacingOccurrences(of: "color", with: "S") + String(str_toGiftName)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.gradientShared(colors: UIColor.endColor(), size: CGSize(width: 72, height: 32)), for: .normal)
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.backgroundColor = UIColor(hex: (String(str_hiddenText) + String(str_managerValue)))
        //: tf.textColor = UIColor.elementColor()
        tf.textColor = UIColor.elementColor()
        //: tf.font = UIFont.rugularSize(fontSize: 16)
        tf.font = UIFont.rugularSize(fontSize: 16)
        ...
        tf.placeholder = (String(str_resumeMidValue.prefix(7)) + String(str_leadingText.suffix(6))).localized
    ...
    private lazy var unlessEditingView: UIView = {
        //: let image = UIImage.nameTitle(name: "icon_search_sea")
        let image = UIImage.nameTitle(name: (String(str_makeValue.prefix(5)) + "sear" + String(str_assetName.prefix(6))))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(TurnOutView.self, forCellReuseIdentifier: TurnOutView.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(str_bubbleSizeName)).localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(str_actualValue) + String(str_labelValue))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + kLet_timeText, width: kLet_failureName, height: kLet_appEventDomainName - (14 + kLet_timeText + kLet_soundData)), style: style)
    ...
    private lazy var reqManager: RemoveReactiveCompatible = //: return RemoveReactiveCompatible()
        .init()
    //: }()
}
...
extension DataBorderFieldDelegate {
    // 根据uid查询用户
    ...
    private func scaleValue() {
        ...
        guard searchTF.text?.count ?? 0 > 0 else {
            ...
            self.barLog(showMsg: (String(str_rawText) + String(str_shareData.prefix(5)) + "y UID").localized)
            //: return
            return
        }
        ...
        reqManager.fileCompletion(uid: searchTF.text ?? "") { [weak self] _, _, _ in
            //: ProgressHUD.dismiss()
...
extension DataBorderFieldDelegate: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: TurnOutView.className(), for: indexPath) as! TurnOutView
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.clickModel(model: model)
...
extension DataBorderFieldDelegate: UITextFieldDelegate {
...
extension DataBorderFieldDelegate: JXPagingViewListViewDelegate {
