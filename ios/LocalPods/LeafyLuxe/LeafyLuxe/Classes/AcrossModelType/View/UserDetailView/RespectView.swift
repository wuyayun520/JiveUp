...
fileprivate let str_enableLineValue:[UInt8] = [0x7c,0x7b,0x7c,0x61,0x3d,0x76,0x7a,0x71,0x70,0x67,0x2f,0x3c,0x35,0x7d,0x74,0x66,0x35,0x7b,0x7a,0x61,0x35,0x77,0x70,0x70,0x7b,0x35,0x7c,0x78,0x65,0x79,0x70,0x78,0x70,0x7b,0x61,0x70,0x71]

/*: "icon_userdetail_header_shadow" :*/
fileprivate let str_finishEmptyText:[Character] = ["i","c","o","n","_","u","s","e","r","d","e","t","a","i","l","_","h","e","a","d","e","r","_","s","h","a","d","o"]
fileprivate let str_labDataTitle:String = "progress"

/*: "icon_lounge" :*/
fileprivate let str_contentOmitName:String = "icon_menu else if"
fileprivate let str_visibleName:[Character] = ["l","o","u","n","g","e"]

/*: "Official" :*/
fileprivate let str_sendPopValue:String = "style ifOfficia"
fileprivate let str_mainRefreshName:[Character] = ["l"]

/*: "Reviewing" :*/
fileprivate let str_editValue:String = "return label pushRevie"

/*: "#FF506D" :*/
fileprivate let str_deadlineName:[Character] = ["#","F","F","5","0","6","D"]

/*: "icon_user_detail_intimate_header" :*/
fileprivate let str_hiddenText:[UInt8] = [0x53,0x59,0x55,0x54,0x65,0x4f,0x49,0x5f,0x48,0x65,0x5e,0x5f,0x4e,0x5b,0x53,0x56,0x65,0x53,0x54,0x4e,0x53,0x57,0x5b,0x4e,0x5f,0x65,0x52,0x5f,0x5b,0x5e,0x5f,0x48]

private func letImage(link num: UInt8) -> UInt8 {
    return num ^ 58
}

/*: "?toUid= :*/
fileprivate let str_eventTableValue:[Character] = ["?","t","o","U","i","d","="]

/*: "ClearPlayerDelegate" :*/
fileprivate let str_rankMaleName:String = "TUserDeheight as size let"
fileprivate let str_barRoundTitle:String = "image name let always nottailH"
fileprivate let str_skipName:String = "female playerItemCell"

/*: "#FFFFFF" :*/
fileprivate let str_progressOfName:String = "#FFFFadjustment button observer height"
fileprivate let str_greetTitle:String = "background"

/*: "#BDB8B5" :*/
fileprivate let str_normalName:String = "as self#BDB8B5"

/*: "head_detail_bottom" :*/
fileprivate let str_actionName:String = "hegiftd"
fileprivate let str_makeTitle:String = "itemttitemm"

/*: "#2DF2FF" :*/
fileprivate let str_fillContent:[Character] = ["#","2","D","F","2","F","F"]

/*: "Live" :*/
fileprivate let str_frameCellCalendarData:String = "Liveview background color name"

/*: "#48FF1E" :*/
fileprivate let str_customModelData:String = "#48FF1Egift path"

/*: "Online" :*/
fileprivate let str_sizeAddReturnContent:[Character] = ["O","n","l","i","n","e"]

/*: "#FF4B7F" :*/
fileprivate let str_decisionData:String = "to view#FF4B7F"

/*: "busy" :*/
fileprivate let str_directionName:String = "failureusy"
...
let kLet_domainViewName = (302.0 + kLet_messageSumervalName)

//: class TalkingUserDetailHeaderView: UIView {
class RespectView: UIView {
    //: var VideoCallBlock: (() -> Void)?
    var VideoCallBlock: (() -> Void)?
    //: private var userInfoModel = TalkingUserInfoModel()
    private var userInfoModel = HeadTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_enableLineValue.map{$0^21}, encoding: .utf8)!)
    }
    ...
    private lazy var shadowBgView: UIImageView = {
        //: let v = UIImageView(image: UIImage.nameTitle(name: "icon_userdetail_header_shadow"))
        let v = UIImageView(image: UIImage.nameTitle(name: (String(str_finishEmptyText) + str_labDataTitle.replacingOccurrences(of: "progress", with: "w"))))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: kLet_domainViewName - 15, width: kLet_failureName, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.nameTitle(name: (String(str_contentOmitName.prefix(5)) + String(str_visibleName)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.setTitle((String(str_sendPopValue.suffix(7)) + String(str_mainRefreshName)).localized, for: .normal)
    ...
    private lazy var onlineImgV: OfTalkingButton = {
    ...
    private lazy var liveView: OfStreamsView = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: TalkingPushManager.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            TalkingPushManager.share.levelType(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(str_editValue.suffix(5)) + "wing").localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(str_deadlineName)))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .magnitudeimateBy(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: OfTalkingButton = {
        //: let btn = TalkingButton()
        let btn = OfTalkingButton()
        //: btn.setImage(UIImage.nameTitle(name: "icon_user_detail_intimate_header"), for: .normal)
        btn.setImage(UIImage.nameTitle(name: String(bytes: str_hiddenText.map{letImage(link: $0)}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(adjacentOrMoveColor), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension RespectView {
    /// 查看视频
    //: private func goToVideo() {
    private func networkVideo() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = HaltViewController(videoPath: self.userInfoModel.videoUrl)
        //: if SubLabelReactiveCompatible.share.loginUid != self.userInfoModel.uid {
        if SubLabelReactiveCompatible.share.loginUid != self.userInfoModel.uid {
    ...
    private func totalestacy(_ index: Int) {
        ...
        let vc = ConversationViewDelegate(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.nowController()?.navigationController?.pushViewController(vc, animated: true)
    }

    //: @objc func intimateBtnClick() {
    @objc func adjacentOrMoveColor() {
        //: let url = TalkingWebManager.getFullUrl(urlType: .Intimate) + "?toUid=\(self.userInfoModel.uid)"
        let url = SizeManager.magnitudeeractionAdd(urlType: .Intimate) + (String(str_eventTableValue)) + "\(self.userInfoModel.uid)"
...
extension RespectView: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: ClearPlayerDelegate.self, reuseIdentifier: (String(str_rankMaleName.prefix(7)) + String(str_barRoundTitle.suffix(5)) + "eader" + String(str_skipName.suffix(8))))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        if index == 0 && userInfoModel.sex == CheckedUTF8Initializable.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            //: bannerCell.setVideoView()
            bannerCell.cellOff()
            //: bannerCell.configModel(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            bannerCell.constraint(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.bgImgView.overMake(urlStr: urlStr, placeImg: UIImage.priceObjectOpen())
        }
    ...
    func jxBanner(_: JXBannerType, centerIndex: Int, centerCell _: UICollectionViewCell) {
        //: guard userInfoModel.uid == SubLabelReactiveCompatible.share.loginUid else {
        guard userInfoModel.uid == SubLabelReactiveCompatible.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(str_progressOfName.prefix(5)) + str_greetTitle.replacingOccurrences(of: "background", with: "FF")))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(str_normalName.suffix(7))))!
...
extension RespectView: JXBannerDelegate {
    ...
    public func jxBanner(_: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == CheckedUTF8Initializable.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            networkVideo()
            ...
            if userInfoModel.sex == CheckedUTF8Initializable.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension RespectView {
    ...
    func priceBy(_ userModel: HeadTransformable) {
        ...
        if userModel.sex == CheckedUTF8Initializable.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        bannerView.reloadView()
        // vip
        //: let normalImg = UIImage.nameTitle(name: "head_detail_bottom").withTintColor(.white)
        let normalImg = UIImage.nameTitle(name: (str_actionName.replacingOccurrences(of: "gift", with: "a") + "_detail_b" + str_makeTitle.replacingOccurrences(of: "item", with: "o"))).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = SubLabelReactiveCompatible.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {

            if let vipModel = SubLabelReactiveCompatible.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.overMake(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .requestUser() : .white
...
//        }
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.requestSize(color: UIColor(hex: (String(str_fillContent)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.circle(), for: .normal)
            onlineImgV.setImage(image.chorus(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(str_frameCellCalendarData.prefix(4))).localized, for: .normal)
            ...
            let image = UIImage.requestSize(color: UIColor(hex: (String(str_customModelData.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.circle(), for: .normal)
            onlineImgV.setImage(image.chorus(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((String(str_sizeAddReturnContent)).localized, for: .normal)
            ...
            let image = UIImage.requestSize(color: UIColor(hex: (String(str_decisionData.suffix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.circle(), for: .normal)
            onlineImgV.setImage(image.chorus(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((str_directionName.replacingOccurrences(of: "failure", with: "b")).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if SubLabelReactiveCompatible.share.loginUid != userModel.uid &&
        if SubLabelReactiveCompatible.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.immediate(liveUrl: userModel.livePlayUrl)
        }
    }
    ...
    private func camera() {
        ...
        liveView.snp.makeConstraints { make in
            ...
            make.top.equalTo(kLet_messageSumervalName + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(kLet_messageSumervalName + 15)
