...
/*: "btn_me_edit" :*/
fileprivate let str_shareName:String = "btn_property layer"
...
class TaskViewCell: UITableViewCell {
	var frameTotal: Int = 29
	var viewPrepareTotal: Double = 55.8
	var listFeeArray: [AnyHashable] = []
	var collisionCourseDictionary: [AnyHashable: String] = [:]
    ...
    override func awakeFromNib() {
        ...
        super.awakeFromNib()
            ...
            if (logutBtn.superview != nil && logutBtn.superview!.isHidden) && (logutBtn.tintAdjustmentMode == .dimmed) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let penLoad = HideNumberView(frame: logutBtn.frame.offsetBy(dx: CGFloat(104.39), dy: CGFloat(77)))
            
            penLoad.playerInsideNumber = { [self] appearCount in
            self.frameTotal = appearCount
            
            var penLoad = index
            penLoad &*= 6
            if penLoad > self.frameTotal {
                self.frameTotal = penLoad
            }
            
            return self.frameTotal
            }
            penLoad.arraySum = { [self] feeQuantity in
            self.viewPrepareTotal = feeQuantity
            
            return self.viewPrepareTotal
            }
            penLoad.sectionArray = { [self] meArray in
            self.listFeeArray = meArray
            
            guard var value = self.listFeeArray as? [String] else {
                return nil
            }
            return value
            }
            penLoad.labelDictionary = { [self] backDictionary in
            self.collisionCourseDictionary = backDictionary
            
            guard var value = self.collisionCourseDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                logutBtn.addSubview(penLoad)
            }
    ...
    lazy var editImag: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.nameTitle(name: (String(str_shareName.prefix(4)) + "me_edit")))
    ...
    lazy var detailLB: UILabel = {
        ...
        //: lb.font = UIFont.magnitudeimateBy(type: .Medium, fontSize: 16)
        lb.font = UIFont.magnitudeimateBy(type: .Medium, fontSize: 16)
        //: lb.textColor = UIColor.dataViewPush()
        lb.textColor = UIColor.dataViewPush()
...
extension TaskViewCell {
    ...
    func ingredient(titile: String, row: Int, isLast: Bool) {
    ...
    //: func setArrows() {
    func playerInfo() {
        //: titleBLB.font = UIFont.magnitudeimateBy(type: .Medium, fontSize: 15)
        titleBLB.font = UIFont.magnitudeimateBy(type: .Medium, fontSize: 15)
        //: detailLB.font = UIFont.magnitudeimateBy(type: .Medium, fontSize: 15)
        detailLB.font = UIFont.magnitudeimateBy(type: .Medium, fontSize: 15)
        //: editImag.isHidden = false
        editImag.isHidden = false
