...
fileprivate let str_noValue:[UInt8] = [0x8c,0x8b,0x8c,0x91,0xcd,0x86,0x8a,0x81,0x80,0x97,0xdf,0xcc,0xc5,0x8d,0x84,0x96,0xc5,0x8b,0x8a,0x91,0xc5,0x87,0x80,0x80,0x8b,0xc5,0x8c,0x88,0x95,0x89,0x80,0x88,0x80,0x8b,0x91,0x80,0x81]

/*: "img_loading_lose" :*/
fileprivate let str_buttonModelTitle:[Character] = ["i","m","g","_"]
fileprivate let str_sectionHiddenValue:String = "toad"
...
// __CLOSE_PRINT__
//
//  PreviewRecognizerDelegate.swift
//  AbroadTalking
//
...
class PreviewRecognizerDelegate: UICollectionViewCell {
	var pathClose: Bool = true
	var viewSum: Double = -88.0
	var namedropTitle: String = "data"
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_noValue.map{$0^229}, encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
		...
		//: if_extract_code "birdSEyeView" begin
		...
		if !birdSEyeView {
		    ...
		    birdSEyeView = isValue
		}
		
		//: if_extract_code "birdSEyeView" end
		
            if (birdSEyeView) && (self.bounds.size.width == 140.51) {
            ...
            self.pathClose = turnOutOn
            
            self.pathClose = !self.pathClose
            return self.pathClose
            }
            feedbackCurrent.headerCount = { [self] languageInterval in
            self.viewSum = languageInterval
            
                self.viewSum -= 1
                if self.viewSum != 99 {
                    self.viewSum = self.viewSum + 1
                }
            return self.viewSum
            }
            feedbackCurrent.ageName = { [self] picText in
            self.namedropTitle = picText
            
            return self.namedropTitle
            }
            feedbackCurrent.lineArray = { [self] limitIndexArray in
            self.emptyArray = limitIndexArray
            ...
            return value
                ...
                self.addSubview(feedbackCurrent)
            }
...
extension PreviewRecognizerDelegate {
    //: private func bindInteraction() {
    private func cuttingEdge() {
        //: let singleTap = UITapGestureRecognizer(target: self, action: #selector(singleTapAction))
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(fromControlAction))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(on)).then {
...
extension PreviewRecognizerDelegate {
    ...
    func onColor(imgModel: DiningRoomAttendantMeasurable) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(str_buttonModelTitle) + str_sectionHiddenValue.replacingOccurrences(of: "to", with: "lo") + "ing_lose")), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.makeSubviews()
            }
        }
    ...
    func reload(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(str_buttonModelTitle) + str_sectionHiddenValue.replacingOccurrences(of: "to", with: "lo") + "ing_lose")), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.makeSubviews()
        }
    }
...
extension PreviewRecognizerDelegate: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension PreviewRecognizerDelegate {
    ...
    func makeSubviews() {
        ...
        var newW = kLet_failureName
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = kLet_failureName * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > kLet_appEventDomainName {
            //: newH = ScreenHeight
            newH = kLet_appEventDomainName
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * kLet_appEventDomainName / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (kLet_failureName - newW) / 2.0, y: (kLet_appEventDomainName - newH) / 2)
