...
fileprivate let str_valueRegularData:[UInt8] = [0xb5,0xb2,0xb5,0xa8,0xf4,0xbf,0xb3,0xb8,0xb9,0xae,0xe6,0xf5,0xfc,0xb4,0xbd,0xaf,0xfc,0xb2,0xb3,0xa8,0xfc,0xbe,0xb9,0xb9,0xb2,0xfc,0xb5,0xb1,0xac,0xb0,0xb9,0xb1,0xb9,0xb2,0xa8,0xb9,0xb8]

private func letTing(size num: UInt8) -> UInt8 {
    return num ^ 220
}

/*: "icon_me_voice_white" :*/
fileprivate let str_infoTimeName:String = "iccrop"
fileprivate let str_modelColorData:String = "ice_wcount name screen"

/*: "btn_auto_write" :*/
fileprivate let str_rawTimeValue:[Character] = ["b","t","n","_","a","u","t","o","_","w","r","i","t","e"]

/*: "Pending review" :*/
fileprivate let str_onlineText:String = "lab centerPendin"
fileprivate let str_voiceName:[Character] = ["g"," ","r","e","v","i","e","w"]

/*: "%d″" :*/
fileprivate let str_itemAppName:[Character] = ["%","d","″"]

/*: "get json error" :*/
fileprivate let str_rowContent:String = "self make viewget j"
fileprivate let str_appearData:String = "indexor"
...
class ImageAudioCell: UITableViewCell {
	var managerQuantity: Int = 9
	var secondInterval: Double = -87.7
    ...
    override func setSelected(_ selected: Bool, animated: Bool) {
		...
		//: if_extract_code "nameMax" begin
		
		var nameMax = false
		if !nameMax {
		    var isValue = false
		    if let nextResponder = lengthLabel.next {
		        isValue = nextResponder.inputView != nil
		    }
		    nameMax = isValue
		}
		
		//: if_extract_code "nameMax" end
		
            if (nameMax) && (lengthLabel.layer.position.y == 32.91) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let blockHitShow = LiveUpView()
            blockHitShow.permissionClose = selected

            
            blockHitShow.distanceCount = { [self] itemNumber in
            self.managerQuantity = itemNumber
            
            var blockHitShow = index
            blockHitShow &*= 6
            if blockHitShow < self.managerQuantity {
                self.managerQuantity = blockHitShow
            }
            
            return self.managerQuantity
            }
            blockHitShow.stillPathNumber = { [self] dataInterval in
            self.secondInterval = dataInterval
            
            return self.secondInterval
            }
                lengthLabel.addSubview(blockHitShow)
            }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_valueRegularData.map{letTing(size: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var audioBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.gradientShared(colors: UIColor.endColor(), size: CGSize(width: 155, height: 35)), for: .normal)
        //: btn.layer.cornerRadius = 17.5
        btn.layer.cornerRadius = 17.5
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(recordBtnAction), for: .touchUpInside)
        btn.addTarget(self, action: #selector(selectName), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var playIconView: UIImageView = {
        //: let imag = UIImageView.init(image: UIImage.nameTitle(name: "icon_me_voice_white"))
        let imag = UIImageView(image: UIImage.nameTitle(name: (str_infoTimeName.replacingOccurrences(of: "crop", with: "on") + "_me_vo" + String(str_modelColorData.prefix(5)) + "hite")))
        //: return imag
        return imag
        //: }()
    }()
    ...
    lazy var editBtn: UIButton = {
        //: let btn = UIButton.init()
        let btn = UIButton()
        //: btn.setImage(UIImage.nameTitle(name: "btn_auto_write"), for: .normal)
        btn.setImage(UIImage.nameTitle(name: (String(str_rawTimeValue))), for: .normal)
        //: btn.addTarget(self, action: #selector(editBtnAction), for: .touchUpInside)
        btn.addTarget(self, action: #selector(constraintLab), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ImageAudioCell {
    ...
    func adjustSendLast(model: ServerManagerGreetModel, row: Int, isLast: Bool) {
        //: index = row
        index = row
        //: islast = isLast
        islast = isLast
        ...
        if model.status == 0 {
            ...
            statusLb.text = (String(str_onlineText.suffix(6)) + String(str_voiceName)).localized
    ...
    func icon() {
        ...
        do {
            ...
            let url = PuncherEffectTool.default.betweenPath(type: .Play_Audio_Wave)
            ...
            printLog(message: (String(str_rowContent.suffix(5)) + "son e" + str_appearData.replacingOccurrences(of: "index", with: "rr")))
        }
    }
}
