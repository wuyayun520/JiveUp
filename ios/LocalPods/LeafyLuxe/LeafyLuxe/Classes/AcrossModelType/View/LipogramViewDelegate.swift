...
fileprivate let str_logTextData:[UInt8] = [0x9d,0x9a,0x9d,0x80,0xdc,0x97,0x9b,0x90,0x91,0x86,0xce,0xdd,0xd4,0x9c,0x95,0x87,0xd4,0x9a,0x9b,0x80,0xd4,0x96,0x91,0x91,0x9a,0xd4,0x9d,0x99,0x84,0x98,0x91,0x99,0x91,0x9a,0x80,0x91,0x90]

private func leadingBack(view num: UInt8) -> UInt8 {
    return num ^ 244
}

/*: "Edit Title" :*/
fileprivate let str_objectIndexTitle:String = "data triumph userEdit Ti"
fileprivate let str_phoneData:[Character] = ["t","l","e"]

/*: "0/ :*/
fileprivate let str_allDataName:[Character] = ["0","/"]

/*: "Finish" :*/
fileprivate let str_timeValue:String = "leading share pageFinish"

/*: "btn_delete" :*/
fileprivate let str_titleColorName:[Character] = ["b","t","n","_","d","e","l","e","t"]
fileprivate let str_slideName:String = "menu"

/*: / :*/
fileprivate let str_byAppName:String = "/"
...
class LipogramViewDelegate: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(accumulation))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_logTextData.map{leadingBack(view: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(str_objectIndexTitle.suffix(7)) + String(str_phoneData)).localized
    ...
    lazy var inputTView: TextScrollView = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(str_timeValue.suffix(6))).localized, for: .normal)
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.nameTitle(name: (String(str_titleColorName) + str_slideName.replacingOccurrences(of: "menu", with: "e"))), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(characterImage), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension LipogramViewDelegate {
...
extension LipogramViewDelegate: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = eventFor(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func eventFor(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
