...
public protocol EqualLayoutDelegate: NSObject {
    /// 文字内容for cell
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, textForItemAt indexPath: IndexPath) -> String
    func tagWithAtomicCount85(_ layout: RowTagLayout, collectionView: UICollectionView, textForItemAt indexPath: IndexPath) -> String
    /// section head footer size
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, sizeForSupplementaryElementOfKind kind: String, at section: Int) -> CGSize
    func gift(_ layout: RowTagLayout, collectionView: UICollectionView, sizeForSupplementaryElementOfKind kind: String, at section: Int) -> CGSize

    ///  标签的内边距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, tagInnerMarginForItemAt indexPath: IndexPath) -> CGFloat
    func attention(_ layout: RowTagLayout, collectionView: UICollectionView, tagInnerMarginForItemAt indexPath: IndexPath) -> CGFloat
    
    /// 底部的间距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, heightBottomAt section: Int) -> CGFloat
    func fit(_ layout: RowTagLayout, collectionView: UICollectionView, heightBottomAt section: Int) -> CGFloat

    
    /// 左右的间距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, leftSapceAt section: Int) -> CGFloat
    func infoButton(_ layout: RowTagLayout, collectionView: UICollectionView, leftSapceAt section: Int) -> CGFloat
...
open class RowTagLayout: UICollectionViewLayout {
	var inputInterval: Int = 15
	var stackTotal: Double = 71.2
...
//    open var tagInnerMargin: CGFloat = 25
    ...
    open var itemHeight: CGFloat = 30
    // 标签的字体
    //: open var itemFont = UIFont.magnitudeimateBy(type: .Regular, fontSize: 15)
    open var itemFont = UIFont.magnitudeimateBy(type: .Regular, fontSize: 15)

    //: open weak var delegate: TagLayoutDelegate?
    open weak var delegate: EqualLayoutDelegate?
//    var titles = [String]()
    /// tag 内容的对齐方式
    //: open var contentAlignment = TagContentAlignment.left
    open var contentAlignment = HairTextAlignment.left
    ...
    override open func prepare() {
        ...
        for section in 0 ..< sections {
            ...
            let sectionHeadSize = delegate.gift(self, collectionView: collectionView, sizeForSupplementaryElementOfKind: UICollectionView.elementKindSectionHeader, at: section)
            
            //: let bottomSpace = delegate.tagLayout(self, collectionView: collectionView, heightBottomAt: section)
            let bottomSpace = delegate.fit(self, collectionView: collectionView, heightBottomAt: section)

            //: let itemLeftSpace = delegate.tagLayout(self, collectionView: collectionView, leftSapceAt: section)
            let itemLeftSpace = delegate.infoButton(self, collectionView: collectionView, leftSapceAt: section)
            ...
            for item in 0 ..< rows {
                ...
                let text = delegate.tagWithAtomicCount85(self, collectionView: collectionView, textForItemAt: indexPath)
                //: let tagInnerMargin = delegate.tagLayout(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                let tagInnerMargin = delegate.attention(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                //: let tagWidth = textWidth(text) + tagInnerMargin
                let tagWidth = cover(text) + tagInnerMargin
...
//            contentHeight = contentHeight + sectionFooterSize.height
		...
		//: if_extract_code "blockInput" begin
		
		var blockInput = false
		if !blockInput {
		    var isValue = false
		    if let nextResponder = collectionView.next {
		        isValue = nextResponder.inputView != nil
		    }
		    blockInput = isValue
		}
		
		//: if_extract_code "blockInput" end
		
            if (blockInput) && (collectionView.layer.position.y == 32.91) {
            ...
            self.inputInterval = itemNumber
            
            var blockHitShow = sections
            blockHitShow &*= 6
            if blockHitShow < self.inputInterval {
                self.inputInterval = blockHitShow
            }
            
            return self.inputInterval
            }
            blockHitShow.stillPathNumber = { [self] dataInterval in
            self.stackTotal = dataInterval
            
            return self.stackTotal
            }
                collectionView.addSubview(blockHitShow)
            }
...
//        return visibleLayoutAttributes
    ...
    func cover(_ text: String) -> CGFloat {
