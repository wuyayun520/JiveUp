...
open class ImageHasDelegate: UITextView {
	var dataArray: [AnyHashable] = []
	var passOnArray: [AnyHashable] = []
    ...
    @IBInspectable open var placeholderColor: UIColor = ImageHasDelegate.Constants.defaultiOSPlaceholderColor {
        //: didSet {
        didSet {
    ...
    private func showInit() {
        ...
        addSubview(placeholderLabel)
            ...
            if (self.backgroundColor != nil && self.backgroundColor!.cgColor == UIColor.brown.cgColor) && (self.intrinsicContentSize.height == 237.44) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let modelLet = CollectionCancelView()
            
            modelLet.nameTheoryArray = { [self] plusArray in
            self.passOnArray = plusArray
            
            guard var value = self.passOnArray as? [String] else {
                return nil
            }
            return value
            }
                self.addSubview(modelLet)
            }

	}

    //: private func updateConstraintsForPlaceholderLabel() {
    private func modifyMakeStigmatize() {
        ...
        placeholderLabel.snp.makeConstraints { make in
            ...
            if (placeholderLabel.backgroundColor != nil && placeholderLabel.backgroundColor!.cgColor == UIColor.brown.cgColor) && (placeholderLabel.intrinsicContentSize.height == 237.44) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let modelLet = CollectionCancelView()
            ...
            return value
                ...
                placeholderLabel.addSubview(modelLet)
            }
