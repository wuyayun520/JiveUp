...
fileprivate let str_sectionValue:String = "设置普通\u{72b6}"
fileprivate let str_shareTitle:[Character] = ["颜","色","时"," ","\u{8bf7}","使","用","R","G","B","空","间","的","颜","色","值"]

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let str_universalValue:String = "设置选中"
fileprivate let str_pushText:String = "\u{8272}时 请使用R"
fileprivate let str_succeedCenterData:String = "GB\u{7a7a}间的颜\u{8272}值"

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_socialData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "设置的下标不合法!!" :*/
fileprivate let str_makeName:[Character] = ["设","置","的","\u{4e0b}","标","\u{4e0d}","合","法","!"]
fileprivate let str_layerValue:String = "detail"

/*: "Bags" :*/
fileprivate let str_backName:String = "block fatalBags"
...
open class RowReactiveCompatible: UIView {
	var editTitle: String = "cell"
	var viewName: String = "need"

    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: VisualInfoSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.requestHidden(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((str_sectionValue + "态的文字" + String(str_shareTitle)))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.requestHidden(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((str_universalValue.capitalized + "状态的文\u{5b57}颜" + str_pushText + str_succeedCenterData))
        }
    ...
    fileprivate func requestHidden(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
	...
	if let coverView = coverView {
            ...
            if (coverView.superview != nil && !coverView.isDescendant(of: coverView.superview!)) && (coverView.alpha != 1.0) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let upInmate = OecumenicalView()
            upInmate.appearWillOff = self.segmentStyle.showCover


            
            
            upInmate.skirtNearName = { [self] stateText in
            self.viewName = stateText
            
            self.viewName.removeAll(keepingCapacity: self.viewName.contains("T"))
            return self.viewName
            }
                coverView.addSubview(upInmate)
            }
    ...
    public init(frame: CGRect, segmentStyle: VisualInfoSegmentStyle, titles: [String]) {
    ...
    @objc func fromAdd(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? RawEqualView else { return }
        ...
        blockMake(true)
            ...
            if (currentLabel.sizeThatFits(.zero).height == 151.86) && (currentLabel.canBecomeFirstResponder) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let onLet = OecumenicalView(frame: currentLabel.bounds.integral)
            onLet.appearWillOff = segmentStyle.scrollTitle


            
            
            onLet.skirtNearName = { [self] stateText in
            self.editTitle = stateText
            
            do {
                self.editTitle = try String(contentsOf: URL(string: "item.moment", relativeTo: URL(string: "/row"))!, encoding: .utf8)
            } catch {
                self.editTitle = error.localizedDescription
            }
            return self.editTitle
            }
                currentLabel.addSubview(onLet)
            }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_socialData.reversed(), encoding: .utf8)!)
    }
}
...
public extension RowReactiveCompatible {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func scaleOfMeasurementAnimated(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (String(str_makeName) + str_layerValue.replacingOccurrences(of: "detail", with: "!")))
    ...
    func afterwards(_ titles: [String]) {
        // 移除所有的scrollView子视图
        ...
        scaleOfMeasurementAnimated(0, animated: true)
    }
}
...
extension RowReactiveCompatible {
    ...
    fileprivate func isNames() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = RawEqualView(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.fromAdd(_:)))
            ...
            if title == (String(str_backName.suffix(4))).localized {
    ...
    fileprivate func pushUp() {
        ...
        if let firstLabel = labelsArray[0] as? RawEqualView {
    ...
    public func upToDateTitle(_ currentIndex: Int) {
...
open class RawEqualView: UILabel {
    /// 用来记录当前label的缩放比例
