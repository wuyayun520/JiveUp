...
/*: "num" :*/
fileprivate let str_showSizeName:[Character] = ["n","u","m"]
...
@objc public enum MinQuickLookable: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct RillModelType: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// MinQuickLookable
    ...
    mutating func juggle(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct TalkingMeasurable: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(str_showSizeName))
    }
}
