...
fileprivate let str_viewFailureEqualTitle:[UInt8] = [0xa6,0xa1,0xa6,0xbb,0xe7,0xac,0xa0,0xab,0xaa,0xbd,0xf5,0xe6,0xef,0xa7,0xae,0xbc,0xef,0xa1,0xa0,0xbb,0xef,0xad,0xaa,0xaa,0xa1,0xef,0xa6,0xa2,0xbf,0xa3,0xaa,0xa2,0xaa,0xa1,0xbb,0xaa,0xab]

private func nookAndCranny(video num: UInt8) -> UInt8 {
    return num ^ 207
}
...
@objc protocol PictureViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func lease()

    //: @objc optional func endScroll()
    @objc optional func toMale()
}

//: class TalkingDanmuMsgListTableView: UIView {
class DeleteTableView: UIView {
    ...
    var msgArray = [DismissTransformable]() /// 消息数组(数据源)
    //: var  tempMsgArray = Array<TalkingLiveRoomDanmuModel>()
    var tempMsgArray = [DismissTransformable]() /// 用于存储消息还未刷新到tableView的时候接收到的消息
    //: var inPending = false
    var inPending = false /// 是否处于爬楼状态
    //: var refreshTimer: Timer?
    var refreshTimer: Timer? /// 刷新定时器
    //: var reloadType: ReloadLiveMsgRoomType?
    var reloadType: RoomPropertyProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_viewFailureEqualTitle.map{nookAndCranny(video: $0)}, encoding: .utf8)!)
    }
...
extension DeleteTableView {
    ...
    func withTo() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(positionAction), userInfo: nil, repeats: true)
    }
...
extension DeleteTableView {
    ...
    func partyBlock(msgModel: DismissTransformable?) {
    ...
    func closeSize() {
        ...
        if AllHeight > kLet_netLiveData {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < kLet_netLiveData {
                ...
                self.msgTableView.height = kLet_netLiveData
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = kLet_netLiveData - AllHeight
        ...
        beginAnimated(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func beginAnimated(animated: Bool) {
...
extension DeleteTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func upWith(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.toMale?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(kLet_textData)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = AtViewCell.atomicCount85(tableView: tableView, msg: msgModel, indexPath: indexPath)
