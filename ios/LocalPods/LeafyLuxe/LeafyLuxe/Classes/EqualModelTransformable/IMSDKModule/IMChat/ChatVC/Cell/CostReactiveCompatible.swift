...
fileprivate let str_directionAcceptData:[UInt8] = [0xcc,0xd1,0xcc,0xd7,0x8b,0xc6,0xd2,0xc7,0xc8,0xd5,0x9d,0x8c,0x83,0xcb,0xc4,0xd6,0x83,0xd1,0xd2,0xd7,0x83,0xc5,0xc8,0xc8,0xd1,0x83,0xcc,0xd0,0xd3,0xcf,0xc8,0xd0,0xc8,0xd1,0xd7,0xc8,0xc7]

fileprivate func writtenMatter(across num: UInt8) -> UInt8 {
...
fileprivate let str_buttonTextSpringName:[Character] = ["i","c","o","n","_","c","h","a","t","_","t","r","a","n","s","l","a","t"]
fileprivate let str_yetText:[Character] = ["e","_","n","o","r"]

/*: "icon_chat_translate_pre" :*/
fileprivate let str_enableName:String = "blind screen request counticon_ch"
fileprivate let str_momentValue:String = "at_tshow let as"
fileprivate let str_makeFileProgressData:[Character] = ["e","_","p","r","e"]

/*: "icon_chat_miss" :*/
fileprivate let str_methodValue:String = "icon_cmake count digital"
fileprivate let str_onTapName:[Character] = ["h","a","t"]
fileprivate let str_pushName:String = "_misspost network"

/*: "icon_chat_yes" :*/
fileprivate let str_extraTitle:String = "icohidden"
fileprivate let str_priorityWillTitle:[Character] = ["y","e","s"]
...
class CostReactiveCompatible: TextMessageCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: VanguardCellData?
    var textData: VanguardCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_directionAcceptData.map{writtenMatter(across: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.nameTitle(name: (String(str_buttonTextSpringName) + String(str_yetText))), for: .normal)
        //: btn.setImage(UIImage.nameTitle(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.nameTitle(name: (String(str_enableName.suffix(7)) + String(str_momentValue.prefix(4)) + "ranslat" + String(str_makeFileProgressData))), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(zone), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension CostReactiveCompatible {
    ...
    override func fill(with data: TCommonCellData) {
        ...
        super.fill(with: data)
        //: textData = data as? VanguardCellData
        textData = data as? VanguardCellData
        ...
        //: let maxWidth = ManagerReactiveCompatible.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = ManagerReactiveCompatible.repossessMoreScreen() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.nameTitle(name: (String(str_methodValue.prefix(6)) + String(str_onTapName) + String(str_pushName.prefix(5))))
            //: } else if textData.translateStatus == .MsgTranslate_succeed {
        } else if textData.translateStatus == .MsgTranslate_succeed {
            //: self.transShowView.stopAnimating()
            self.transShowView.stopAnimating()
            //: self.transStatusImagV.image = UIImage.nameTitle(name: "icon_chat_yes")
            self.transStatusImagV.image = UIImage.nameTitle(name: (str_extraTitle.replacingOccurrences(of: "hidden", with: "n") + "_chat_" + String(str_priorityWillTitle)))
        }
    ...
    func behindCall() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(toTap(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func toTap(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
