...
fileprivate let str_lookTitle:[UInt8] = [0xb0,0xb7,0xb0,0xad,0xf1,0xba,0xb6,0xbd,0xbc,0xab,0xe3,0xf0,0xf9,0xb1,0xb8,0xaa,0xf9,0xb7,0xb6,0xad,0xf9,0xbb,0xbc,0xbc,0xb7,0xf9,0xb0,0xb4,0xa9,0xb5,0xbc,0xb4,0xbc,0xb7,0xad,0xbc,0xbd]

private func tickText(ret num: UInt8) -> UInt8 {
    return num ^ 217
}

/*: "icon_chat_lock" :*/
fileprivate let str_stackData:[Character] = ["i","c","o","n","_","c","h","a","t","_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let str_cameraFrameTitle:[Character] = ["i"]
fileprivate let str_priceSendValue:[Character] = ["c","o","n","_","c","h","a","t","_","b","u","r","n","e","d"]

/*: "icon_chat_burned_shan" :*/
fileprivate let str_strokeText:[Character] = ["i","c","o","n","_","c","h","a","t","_"]
fileprivate let str_toPathValue:String = "burevent"
fileprivate let str_managerCurrentValue:[Character] = ["d","_","s","h","a","n"]

/*: "Already burned" :*/
fileprivate let str_bubbleText:String = "Alreaguard background recommend"
fileprivate let str_rangeManagerName:[Character] = ["d","y"," ","b","u","r","n","e","d"]
...
class TalkingCellThen: TextMessageCell {
	var sessionNumber: Int = 80
	var stateTotal: Double = 9.7
	var ordinalCountNumber: Int = 14
	var offInterval: Double = -6.8
    ...
    override func awakeFromNib() {
        ...
        super.awakeFromNib()
            ...
            if (burnedImagV.tag == 1024) && (burnedImagV.alpha != 1.0) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let penLet = ElementView()
            penLet.drawUpMagnitude = { [self] refuseLoadCount in
            self.ordinalCountNumber = refuseLoadCount
            
            return self.ordinalCountNumber
            }
            penLet.popNumber = { [self] firstNumber in
            self.offInterval = firstNumber
            
            return self.offInterval
            }
                burnedImagV.addSubview(penLet)
            }
    ...
    override func setSelected(_ selected: Bool, animated: Bool) {
        ...
        super.setSelected(selected, animated: animated)
            ...
            if (photoView.isMultipleTouchEnabled) && (photoView.layer.contents != nil) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let canvasGroup = ElementView(frame: photoView.bounds.insetBy(dx: CGFloat(0), dy: CGFloat(75)))
            canvasGroup.drawUpMagnitude = { [self] refuseLoadCount in
            self.sessionNumber = refuseLoadCount
            
            return self.sessionNumber
            }
            canvasGroup.popNumber = { [self] firstNumber in
            self.stateTotal = firstNumber
            
            return self.stateTotal
            }
                photoView.addSubview(canvasGroup)
            }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_lookTitle.map{tickText(ret: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        //: imag.alpha = 0.9
        imag.alpha = 0.9
        //: imag.image = UIImage.nameTitle(name: "icon_chat_lock")
        imag.image = UIImage.nameTitle(name: (String(str_stackData)))
    ...
    lazy var burnedImagV: OfTalkingButton = {
        ...
        imgV.setBackgroundImage(UIImage.nameTitle(name: (String(str_cameraFrameTitle) + String(str_priceSendValue))), for: .normal)
        //: imgV.setImage(UIImage.nameTitle(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.nameTitle(name: (String(str_strokeText) + str_toPathValue.replacingOccurrences(of: "event", with: "ne") + String(str_managerCurrentValue))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.spaceBetweenTitleAndImage = 20
        imgV.spaceBetweenTitleAndImage = 20
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(str_bubbleText.prefix(5)) + String(str_rangeManagerName)).localized, for: .normal)
...
extension TalkingCellThen {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)
        //: guard let imageCellData = data as? ShareThen else { return }
        guard let imageCellData = data as? ShareThen else { return }

        //: self.photoView.backgroundColor = .white
        self.photoView.backgroundColor = .white
        //: let intimatePhoto: HypothesisPingModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        let intimatePhoto: HypothesisPingModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            onView(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            onView(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: ShareThen, urlStr: String?, isMosaic: Bool) {
    func onView(data: ShareThen, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.name(image, transFrameExtremeObject: 90)
                        self.photoView.image = UIImage.name(image, transFrameExtremeObject: 90)
                        //: } else {
                    } else {
            ...
            jump(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.clap(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func jump(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.showEqual(urlStr: urlStr) { [weak self] image in
