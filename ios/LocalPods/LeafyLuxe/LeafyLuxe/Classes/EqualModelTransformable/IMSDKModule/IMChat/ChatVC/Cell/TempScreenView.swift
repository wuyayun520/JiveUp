...
fileprivate let str_countLabelData:[UInt8] = [0x5e,0x63,0x5e,0x69,0x1d,0x58,0x64,0x59,0x5a,0x67,0x2f,0x1e,0x15,0x5d,0x56,0x68,0x15,0x63,0x64,0x69,0x15,0x57,0x5a,0x5a,0x63,0x15,0x5e,0x62,0x65,0x61,0x5a,0x62,0x5a,0x63,0x69,0x5a,0x59]

fileprivate func loadLet(profile num: UInt8) -> UInt8 {
...
/*: "move" :*/
fileprivate let str_normalNameImageContent:String = "MOVE"
...
class TempScreenView: UIView {
	var imageText: String = "handle"
	var toolContent: String = "fast"
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_countLabelData.map{loadLet(profile: $0)}, encoding: .utf8)!)
    }
    ...
    func toObject(_ danumuMsgModel: RemoveMeasurable?) {
		...
		if var aboutMultiValue = danumuMsgModel?.bgPic { 
			if var costValue = danumuMsgModel?.bgPic { 
		            ...
		            tillPlayer.endTitle = costValue
		            
		            
		            tillPlayer.modelTitle = { [self] listAppearTitle in
		            self.imageText = listAppearTitle
		            
		            var tillPlayer = aboutMultiValue
		            if let rowIf = tillPlayer.randomElement() {
		                tillPlayer.append(rowIf)
		            }
		            if tillPlayer.hasPrefix(self.imageText) {
		                self.imageText = tillPlayer
		            }
    ...
    @objc func titlePost(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(titlePost(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension TempScreenView: UIGestureRecognizerDelegate {
    ...
    func upPath(for danumuMsgModel: RemoveMeasurable?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = EndThen(model: danumuMsgModel)
        ...
        var frame = CGRect(x: kLet_failureName, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if PropertyLineThen.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = kLet_failureName
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (str_normalNameImageContent.lowercased()))
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + itemView.totalTime / 2) { [unowned self] in
                ...
                modeltoTransmissionChannelChangeShapeDaumu(isFirst)
                //: })
            }
...
//    }
    ...
    func modeltoTransmissionChannelChangeShapeDaumu(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                upPath(for: model, isFirst: isFirst)
...
extension TempScreenView {
    ...
    private func objectSubviews() {
        ...
        firstChannelEmptyBlock = { [unowned self] in
            ...
            modeltoTransmissionChannelChangeShapeDaumu(true)
        }
    }
}
