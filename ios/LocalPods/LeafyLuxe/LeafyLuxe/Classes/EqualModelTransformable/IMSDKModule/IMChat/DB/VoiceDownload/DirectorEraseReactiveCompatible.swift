...
fileprivate let str_viewColorOfValue:String = "Docucolor var task than"
fileprivate let str_withSectionMediaValue:String = "/Usertrue height"
fileprivate let str_topTitle:String = "pic path share not/voice/"

/*: "Documents/User/record/" :*/
fileprivate let str_mValue:String = "Documentlog drop"
fileprivate let str_indicatorName:String = "s/Usemodel extension domain leading label"
fileprivate let str_strokeData:String = "ord/view fail size to object"

/*: "cancelled" :*/
fileprivate let str_managerMakeData:String = "canctopled"

/*: .wav" :*/
fileprivate let str_layerText:[Character] = [".","w","a","v"]
...
// __CLOSE_PRINT__
//
//  DirectorEraseReactiveCompatible.swift
//  LeafyLuxe
//
...
typealias OneKeyGreetDownloadBlock = (_ model: ElectThen) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol ToEndManagerDelegate: NSObjectProtocol {
    /// 下载完成
...
extension DirectorEraseReactiveCompatible {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func match(_ tasksArray: [ElectThen]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.weight()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(str_viewColorOfValue.prefix(4)) + "ments" + String(str_withSectionMediaValue.prefix(5)) + String(str_topTitle.suffix(7)))) {
                //: model.zipFilePath = "\(ConstraintPlayerCacheDefine.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(ConstraintPlayerCacheDefine.viewBegin())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(str_mValue.prefix(8)) + String(str_indicatorName.prefix(5)) + "r/rec" + String(str_strokeData.prefix(4)))) {
                //: model.zipFilePath = "\(ConstraintPlayerCacheDefine.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(ConstraintPlayerCacheDefine.stateError())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            model.taskStatus = .prepare
            //: FirstReactiveCompatible.db_updateVoiceMsg(msgId: model.taskId, status: "\(DownloadTaskStatus.prepare.rawValue)", filePath: model.zipFilePath)
            FirstReactiveCompatible.illegitimacy(msgId: model.taskId, status: "\(AmbitComparable.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func weltUserHandle(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = ElectThen.doingeModel(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? ElectThen()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(str_viewColorOfValue.prefix(4)) + "ments" + String(str_withSectionMediaValue.prefix(5)) + String(str_topTitle.suffix(7)))) {
            //: taskModel.zipFilePath = "\(ConstraintPlayerCacheDefine.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(ConstraintPlayerCacheDefine.viewBegin())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(str_mValue.prefix(8)) + String(str_indicatorName.prefix(5)) + "r/rec" + String(str_strokeData.prefix(4)))) {
            //: taskModel.zipFilePath = "\(ConstraintPlayerCacheDefine.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(ConstraintPlayerCacheDefine.stateError())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension DirectorEraseReactiveCompatible: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive _: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = ElectThen.thirdSize(path: taskModel.zipFilePath)
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.emptyIn(model: taskModel)
    }
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (str_managerMakeData.replacingOccurrences(of: "top", with: "el")) {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    commentSuspend(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.cancelLikeJob(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                commentSuspend(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.resourceCustom(url: task.taskDescription ?? "")
        }
...
extension DirectorEraseReactiveCompatible {
    ...
    func commentSuspend(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(AmbitComparable.pause.rawValue)" : "\(AmbitComparable.error.rawValue)"
            //: FirstReactiveCompatible.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            FirstReactiveCompatible.illegitimacy(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.beyondSystem(model: taskModel)
            }
    ...
    func cancelLikeJob(url: String) {
        ...
        //: case .chatRecord:
        case .chatRecord:
            //: FirstReactiveCompatible.db_deleteVoiceMsg(msgId: taskModel.taskId)
            FirstReactiveCompatible.contextOfUseId(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.nest(model: taskModel)
    ...
    func resourceCustom(url: String) {
        //: guard let taskModel = self.downloadingModelDic[url] else { return }
        guard let taskModel = self.downloadingModelDic[url] else { return }
        //: self.downloadingModelDic.removeValue(forKey: url)
        self.downloadingModelDic.removeValue(forKey: url)
        ...
        let size = ElectThen.thirdSize(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            //: if taskModel.taskTypeId == .chatRecord {
            if taskModel.taskTypeId == .chatRecord {
                //: FirstReactiveCompatible.db_updateVoiceMsg(msgId: taskModel.taskId, status: "\(DownloadTaskStatus.finish.rawValue)")
                FirstReactiveCompatible.illegitimacy(msgId: taskModel.taskId, status: "\(AmbitComparable.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(str_layerText))
            ...
            self.differentialCost(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.beauty(model: taskModel)
            ...
            //: case .chatRecord:
            case .chatRecord:
                //: FirstReactiveCompatible.db_updateVoiceMsg(msgId: taskModel.taskId, status: "\(DownloadTaskStatus.expired.rawValue)")
                FirstReactiveCompatible.illegitimacy(msgId: taskModel.taskId, status: "\(AmbitComparable.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.reduce(model: taskModel)
    ...
    func differentialCost(at path: String, withKeyId keyId: String, withTypeId typeId: HeadHashableRepresentation, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(str_layerText))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    let unzippedData = data.sizeReSkin()
                    ...
                    if typeId == .chatRecord {
                        //: FirstReactiveCompatible.db_updateVoiceMsg(msgId: keyId, filePath: newPath)
                        FirstReactiveCompatible.illegitimacy(msgId: keyId, filePath: newPath)
                    }
