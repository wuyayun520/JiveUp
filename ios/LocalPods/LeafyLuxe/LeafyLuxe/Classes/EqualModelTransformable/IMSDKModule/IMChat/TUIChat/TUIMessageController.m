...
- (void)setupViews {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(menuWillHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onNewMessage:) name:[BorderGrapnelThen gatherUpWithoutNoti] object:nil];
...
- (void)loadMessage {
    ...
    if (self.currentChatType == TalkingIMChatType_group ||
        ...
        [[V2TIMManager sharedInstance] getGroupHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([TempStopThen shared].isConnection) { //推送点进来会报错
                NSString *str = [@"Net Error, Try again later" localized];
                [self barLogWithShowMsg:[str localized]];
            }
        ...
        [[V2TIMManager sharedInstance] getC2CHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([TempStopThen shared].isConnection) {                
                if (SubLabelReactiveCompatible.share.appStatus == 0) {
                    NSString *str = [@"Net Error, Try again later" localized];
                    [self barLogWithShowMsg:[str localized]];
                }
            }
...
- (void)getMessages:(NSArray *)msgs msgCount:(int)msgCount {
    ...
    for (NSInteger k = array.count - 1; k >= 0; --k) {
        ...
        if ([userId isEqualToString:SubLabelReactiveCompatible.share.loginUserMode.userID]) {
...
- (void)onNewMessage:(NSNotification *)notification
{
    ...
    if ([msg.sender isEqualToString:SubLabelReactiveCompatible.share.loginUserMode.userID]) {
        //过滤本人发送的消息
        return;
    }
    ...
    if (uiMsgs.count > 0) {
        ...
        if ((self.tableView.contentSize.height - self.tableView.contentOffset.y) < [ManagerReactiveCompatible releaseScreenTable] * 1.5) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //    缓存存在问题待处理
    //    if (height > 0 && ![data isKindOfClass:[AbSelectedThen class]] && ![data isKindOfClass:[NoCellData class]]) {
    //        return height;
    //    }
...
- (void)sendCustomMessage:(TUIMessageCellData *)msg {
    ...
    NSInteger sender = SubLabelReactiveCompatible.share.loginUserMode.userID.integerValue;
    NSString *msgID;
    if (isGroup) {
...
- (void)modifyCustomMessageDataWithExtral:(NSDictionary *)extral cellData:(TUIMessageCellData *)msg {
    ...
    // 更新内存模型
    msg.msgModel = [TitleBlendModel modelWithDic:newExtraDict];
}
...
///   - msg: 图片消息
///   - model: 私密照片模型
- (void)changeIntimatePhotoMsg:(TUIMessageCellData *)msg photoModel:(HypothesisPingModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [HypothesisPingModel image:model quantityimateUser:msg.msgID];
...
///   - msg: 视频消息
///   - model: 私密视频模型
- (void)changeIntimateVideoMsg:(TUIMessageCellData *)msg videoModel:(LabelModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [LabelModel size:model withVideo:msg.msgID];
...
- (TalkingSystemMsgCellData *)transSystemMsgFromDate:(NSDate *)date
{
    if(_msgForDate == nil || fabs([date timeIntervalSinceDate:_msgForDate.timestamp]) > MAX_MESSAGE_SEP_DLAY) {
        ...
        system.contentStr = [NSDate doingNumberimateWithShowDetail:YES date:date];
...
- (void)onLongPressMessageWithCell:(TUIMessageCell *)cell{
    ...
    BOOL hasJumps = NO;
    if ([data isKindOfClass:[MagnitudeernationalismReactiveCompatible class]]) {
        MagnitudeernationalismReactiveCompatible *textCellData = (MagnitudeernationalismReactiveCompatible *)data;
    ...
    // 只有文字才有复制功能
    if (([data isKindOfClass:[VanguardCellData class]] ||
         [data isKindOfClass:[MagnitudeernationalismReactiveCompatible class]]) && hasJumps == NO) {
        // 过滤掉音视频通话文本消息
...
- (NSArray *)requiredResponseWithLongGestureRecognizer {
    return @[[VanguardCellData class],
             [MagnitudeernationalismReactiveCompatible class],
             [TempRoomCellData class],
             [ShareThen class],
             [VideoCellData class],
             [ServerCellData class],
             [RangeCellData class],
             [AcrossCellData class]];
}
...
- (void)onCopyMsg:(id)sender {
    ...
    if ([_menuUIMsg isKindOfClass:[VanguardCellData class]] ||
        [_menuUIMsg isKindOfClass:[MagnitudeernationalismReactiveCompatible class]]) {
        MagnitudeernationalismReactiveCompatible *txtMsg = (MagnitudeernationalismReactiveCompatible *)_menuUIMsg;
...
- (BOOL)canReplyResponseWithLongGestureRecognizer:(TUIMessageCellData *)cellData {
    ...
    if ([cellData isKindOfClass:[ServerCellData class]] || [cellData isKindOfClass:[TempRoomCellData class]] || [cellData isKindOfClass:[AcrossCellData class]]) {
        return YES;
    }
...
- (void)translateMsgWithCellData:(TUIMessageCellData *)cellData {
    ...
    if (SubLabelReactiveCompatible.share.appUserConfigMode.enableTranslate == NO) { // 非会员，无法使用翻译
        [[TalkingPushManager share] showGender];
        return;
    }
    
    if ([cellData isKindOfClass:[MagnitudeernationalismReactiveCompatible class]]) {
        MagnitudeernationalismReactiveCompatible *txtMsg = (MagnitudeernationalismReactiveCompatible *)cellData;
        [self translateMessageToReq:txtMsg andType:0];
        
    } else if ([cellData isKindOfClass:[VanguardCellData class]]) {
        VanguardCellData *txtMsg = (VanguardCellData *)cellData;
...
- (void)translateMessageToReq:(TUIMessageCellData *)txtMsg andType:(int)type {
    ...
    // 调用翻译接口
    [ViewRequestTool formWithMsgId:txtMsg.msgModel.msgInfo.msgId type:type completion:^(BOOL succeed, NSDictionary *dict, NSInteger errorCode) {
        if (!succeed) {
            if (errorCode == 403) { // 非会员，无法使用翻译
                SubLabelReactiveCompatible.share.appUserConfigMode.enableTranslate = NO;
                [[TalkingPushManager share] showGender];
            }
