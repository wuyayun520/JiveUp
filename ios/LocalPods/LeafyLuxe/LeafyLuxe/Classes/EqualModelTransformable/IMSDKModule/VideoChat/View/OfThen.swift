...
fileprivate let str_cameraGiftValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class OfThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_cameraGiftValue.reversed(), encoding: .utf8)!)
    }
...
extension OfThen {
    ...
    override func hitTest(_ point: CGPoint, with _: UIEvent?) -> UIView? {
        ...
        for subview in self.subviews {
            ...
            let itemview: TaskItemView = subview as! TaskItemView
            ...
            if #available(iOS 11.0, *) {
                ...
                if itemview.avatarImage.frame.contains(point) {
                    //: TalkingPushManager.share.func__pushToUserDetailVC(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    TalkingPushManager.share.equalAcross(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    //: break
                    break
                }
                //: } else {
    ...
    func clickInfo(model: RillModelType) {
        //  1.相同礼物时则连击
        //: for  subview in self.subviews {
        for subview in self.subviews {
            //: let itemview: TalkingGiftTrackItemView = subview as! TalkingGiftTrackItemView
            ...
            if appDismiss(giftMsgModel: itemview.giftMsgModel, withGiftMsgModel: model) {
                //: itemview.comboHitWith(giftMsgModel: model)
                itemview.bunkerContext(giftMsgModel: model)
                //: itemview.setGiftMsgModel(model: model)
                itemview.indexWith(model: model)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            let subView: TaskItemView? = self.viewWithTag(tag) as! TaskItemView?
            //: if subView == nil {
            if subView == nil {
                //: self.addNewGiftDanumWith(giftMsgModel: model, atIndex: index)
                self.afterImage(giftMsgModel: model, atIndex: index)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            if index == 1 || index == 2 {
                ...
                let subView1: TaskItemView = self.viewWithTag(tag) as! TaskItemView
                ...
                subView1.buttonCustom(frame: frame)
    ...
    func appDismiss(giftMsgModel: RillModelType, withGiftMsgModel: RillModelType) -> (Bool) {
...
//        subView.showUserInfoViewBlock   = self.showUserInfoViewBlock
        ...
        subView.indexWith(model: giftMsgModel)
        //: self.addSubview(subView)
        self.addSubview(subView)

        //: subView.showGiftAnimate(model: giftMsgModel, animate: true)
        subView.imageAnimate(model: giftMsgModel, animate: true)
    }
}
