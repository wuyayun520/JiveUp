...
fileprivate let str_addBarData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#171717" :*/
fileprivate let str_aspectValue:String = "#1"
fileprivate let str_atRedTitle:String = "71717"

/*: "icon_cameraoff" :*/
fileprivate let str_playerTitle:String = "to officialicon_"
fileprivate let str_feedbackName:String = "aonamename"
...
class ShowWindowView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.buttonEnd(51, 51, 51, 1)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_addBarData.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (str_aspectValue.capitalized + str_atRedTitle.capitalized))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        //: v.contentMode = .scaleAspectFill
        v.contentMode = .scaleAspectFill
        //: v.image = UIImage.nameTitle(name: "icon_cameraoff")
        v.image = UIImage.nameTitle(name: (String(str_playerTitle.suffix(5)) + "camer" + str_feedbackName.replacingOccurrences(of: "name", with: "f")))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension ShowWindowView {
    /// 刷新视频通话背景
    /// - Parameters:
    ///   - urlStr: 背景图片
    ///   - isOpenCamera: 摄像头状态
    //: func refreshBgView(urlStr: String?, isOpenCamera: VideoCameraStatus) {
    func toCamera(urlStr: String?, isOpenCamera: RawTaskCameraStatus) {
...
extension ShowWindowView: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func tapRemote(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, kLet_failureName - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, kLet_appEventDomainName - self.frame.size.height * 0.5)
    ...
    @objc private func theoreticalAccountContainer() {
        //: if self.block != nil {
        if self.block != nil {
            //: self.block!()
            self.block!()
...
extension ShowWindowView {
    ...
    private func context() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(theoreticalAccountContainer))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(tapRemote(pan:)))
        //: pan.delegate = self
        pan.delegate = self
