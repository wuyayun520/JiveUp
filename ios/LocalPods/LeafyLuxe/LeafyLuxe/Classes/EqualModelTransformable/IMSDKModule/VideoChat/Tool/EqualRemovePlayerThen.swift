...
/*: "call_bgm" :*/
fileprivate let str_ageValue:String = "call_bgcolor class"
fileprivate let str_sexData:String = "title"

/*: "call_response_bgm" :*/
fileprivate let str_domainStatusName:String = "call_false make content self block"
fileprivate let str_labelValue:String = "nse_button main"
fileprivate let str_receiveEqualTrueData:[Character] = ["b","g","m"]
...
class EqualRemovePlayerThen: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = CompleteMagnitude.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func byEqual(type: CompleteMagnitude) -> EqualRemovePlayerThen {
        //: let player = TalkingCallBGMPlayer()
        let player = EqualRemovePlayerThen()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.sunnahObserve()
        //: return player
        return player
    }
...
extension EqualRemovePlayerThen {
    //: func play() {
    func infoOfPlay() {
    ...
    private func scrub(notify: Notification) {
    ...
    private func limit(notify: Notification) {
    ...
    private func sunnahObserve() {
        //: var fileName = "call_bgm"
        var fileName = (String(str_ageValue.prefix(7)) + str_sexData.replacingOccurrences(of: "title", with: "m"))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(str_domainStatusName.prefix(5)) + "respo" + String(str_labelValue.prefix(4)) + String(str_receiveEqualTrueData))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = PuncherEffectTool.default.befitType(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension EqualRemovePlayerThen: TXVodPlayListener {
