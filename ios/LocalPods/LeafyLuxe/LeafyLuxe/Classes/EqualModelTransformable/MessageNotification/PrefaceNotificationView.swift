...
fileprivate let str_visualData:[UInt8] = [0x48,0x4d,0x48,0x53,0x7,0x42,0x4e,0x43,0x44,0x51,0x19,0x8,0xff,0x47,0x40,0x52,0xff,0x4d,0x4e,0x53,0xff,0x41,0x44,0x44,0x4d,0xff,0x48,0x4c,0x4f,0x4b,0x44,0x4c,0x44,0x4d,0x53,0x44,0x43]

fileprivate func titleTable(element num: UInt8) -> UInt8 {
...
/*: "Reply" :*/
fileprivate let str_portValue:[Character] = ["R","e","p","l","y"]

/*: "#F95151" :*/
fileprivate let str_momentValue:String = "class manager app source not#F9515"
fileprivate let str_giftOpenIfData:[Character] = ["1"]
...
let kLet_halfMessageData = (kLet_messageSumervalName + 80.0)
//: class TalkingMessageNotificationView: UIView {
class PrefaceNotificationView: UIView {
    //: var startDismissBlock: ((_ model: TalkingConversationModel) -> Void)?
    var startDismissBlock: ((_ model: TempConversationModel) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: PrefaceNotificationView) -> Void)? // 完成移除视图Block
    //: private var currModel = TalkingConversationModel()
    private var currModel = TempConversationModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_visualData.map{titleTable(element: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .magnitudeimateBy(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .magnitudeimateBy(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(str_portValue)).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(str_momentValue.suffix(6)) + String(str_giftOpenIfData))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension PrefaceNotificationView {
    /// 展示视图，5s后自动移除
    //: func show(_ model: TalkingConversationModel?) {
    func observerTag(_ model: TempConversationModel?) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 5) {
                    ...
                    self.dismissAlpha(needAlpha: false)
                }
            }
        }
    }
    ...
    func dismissAlpha(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -kLet_halfMessageData
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func lessen() {
        ...
        if currModel.chatType == .service {
            ...
            //: TalkingPushManager.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            TalkingPushManager.share.eyeglassWearer(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func loadOff(gesture: UISwipeGestureRecognizer) {
...
extension PrefaceNotificationView {
    ...
    func afterReload() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.overMake(urlStr: userInfo.headPic, placeImg: UIImage.barrelFromHeterosexual(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.modelPriceStr(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
            ...
            headImgView.image = UIImage.priceObjectOpen()
            //: iconBorder.isHidden = true
            iconBorder.isHidden = true
        }

        //: msgShowLabel.attributedText = TalkingChatListTableCell.handleShowC2CDetailTextOfMessage(model: currModel)
        msgShowLabel.attributedText = AtTableCell.afterSubmitView(model: currModel)
    }
    ...
    private func giftTo() {
        ...
        self.frame = CGRect(x: 0, y: -kLet_halfMessageData, width: kLet_failureName, height: kLet_halfMessageData)
    ...
    private func clingUp() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(lessen))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(loadOff))
        //: swipe.direction = .up
        swipe.direction = .up
