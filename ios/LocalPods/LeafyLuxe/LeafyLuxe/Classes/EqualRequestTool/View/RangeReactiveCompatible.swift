...
/*: "8075F5" :*/
fileprivate let str_makeData:[Character] = ["8","0","7","5","F","5"]

/*: "9A9B9C" :*/
fileprivate let str_translateValue:String = "make call corner view9A9B9"
fileprivate let str_toEqualValue:[Character] = ["C"]

/*: "kOpacityAnimation" :*/
fileprivate let str_layerContent:String = "kOpacolor model end range if"
fileprivate let str_mainName:String = "Animclass kit kit lab"

/*: "opacity" :*/
fileprivate let str_streamName:[Character] = ["o","p","a","c","i","t","y"]

/*: "forwards" :*/
fileprivate let str_cellMaleText:String = "textrwards"

/*: "easeIn" :*/
fileprivate let str_labViewData:String = "easeInequal data free"

/*: "8C7AF8" :*/
fileprivate let str_toPhotoName:[Character] = ["8","C","7","A","F","8"]
...
class RangeReactiveCompatible: UIView {
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(metalBar(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension RangeReactiveCompatible {
    ...
    func root() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(str_makeData)))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (String(str_translateValue.suffix(5)) + String(str_toEqualValue)))!.withAlphaComponent(0.25).cgColor
            ...
            smallness(index: index, hidden: index == verStr.count ? false : true)
        }
    }

    // 当输入内容等于验证码的长度时候，把输入的验证码回调
    //: @objc func textChage(_ textField: UITextField) {
    @objc func metalBar(_: UITextField) {
        //: func__changeCodeInputView()
        root()
    }

    //: func func__addLabelTapGesture(label: UILabel) {
    func componentAdd(label: UILabel) {
    ...
    fileprivate func smallness(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(str_layerContent.prefix(4)) + "city" + String(str_mainName.prefix(4)) + "ation"))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(toAdd(), forKey: (String(str_layerContent.prefix(4)) + "city" + String(str_mainName.prefix(4)) + "ation"))
        }
    ...
    fileprivate func toAdd() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (String(str_streamName)))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (str_cellMaleText.replacingOccurrences(of: "text", with: "fo")))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (String(str_labViewData.prefix(6)))))
        //: return animation
        return animation
    }
}
...
extension RangeReactiveCompatible {
    ...
    private func eventWill() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (String(str_toPhotoName)))?.cgColor
