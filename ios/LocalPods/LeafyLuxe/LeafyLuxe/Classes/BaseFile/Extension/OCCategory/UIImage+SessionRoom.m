...
#import "UIImage+SessionRoom.h"

//: @implementation UIImage (Compress)
@implementation UIImage (SessionRoom)

//: - (NSData *)compressedImageData {
- (NSData *)length {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.donation;
...
- (UIImage *)magnitudeimate {
    ...
    return [UIImage imageWithData:[self length]];
}
...
- (UIImage *)fromScale:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self file:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self telegram:size];
...
+ (NSData *)largeObject:(UIImage *)image lengthIdentity:(NSInteger)fileSize{
    ...
    NSData *outIMGData = [thumIMG streetSmartContent:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG quantityRelation:scalesize pic:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG streetSmartContent:(fileSize*1024)];
    }
...
- (UIImage *)quantityRelation:(CGSize)targetSize pic:(BOOL)isScale{
...
- (NSData *)streetSmartContent:(NSInteger)fileSize{
...
- (UIImage *)telegram:(CGSize)newSize {
