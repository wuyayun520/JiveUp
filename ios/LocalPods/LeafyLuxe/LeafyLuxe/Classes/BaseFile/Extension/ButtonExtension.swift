...
/*: "recharge" :*/
fileprivate let str_mePagePathData:[Character] = ["r","e","c","h","a","r","g","e"]

/*: "recharge_ :*/
fileprivate let str_editName:[Character] = ["r","e","c","h","a","r","g","e","_"]

/*: ".png" :*/
fileprivate let str_groupValue:String = ".pngin not"

/*: "_ :*/
fileprivate let str_labelName:[Character] = ["_"]

/*: .png" :*/
fileprivate let str_managerName:String = "equal permission title let.png"

/*: ".jpg" :*/
fileprivate let str_failureText:[Character] = [".","j","p","g"]

/*: .jpg" :*/
fileprivate let str_transactionValue:String = "name interaction true false.jpg"
...
enum SessionImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class OfTalkingButton: UIButton {
	var echogramCount: Int = 34
	var noteNumber: Double = 44.5
	var pairNumber: Int = 74
	var receiveMagnitude: Double = 54.1
    ...
    override func titleRect(forContentRect contentRect: CGRect) -> CGRect {
        ...
        //: return super.titleRect(forContentRect: contentRect)
            ...
            if (self.isMultipleTouchEnabled) && (self.layer.contents != nil) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let canvasGroup = ElementView(frame: self.bounds.insetBy(dx: CGFloat(0), dy: CGFloat(75)))
            canvasGroup.drawUpMagnitude = { [self] refuseLoadCount in
            self.echogramCount = refuseLoadCount
            
            return self.echogramCount
            }
            canvasGroup.popNumber = { [self] firstNumber in
            self.noteNumber = firstNumber
            
            return self.noteNumber
            }
                self.addSubview(canvasGroup)
            }
    ...
    var imageAlignment: SessionImageAlignment = .left
    ...
    override func layoutSubviews() {
        ...
        case .right:
            ...
            if (self.isMultipleTouchEnabled) && (self.layer.contents != nil) {
                ...
                let canvasGroup = ElementView(frame: self.bounds.insetBy(dx: CGFloat(0), dy: CGFloat(75)))
            canvasGroup.drawUpMagnitude = { [self] refuseLoadCount in
            self.pairNumber = refuseLoadCount
            
            return self.pairNumber
            }
            canvasGroup.popNumber = { [self] firstNumber in
            self.receiveMagnitude = firstNumber
            
            return self.receiveMagnitude
            }
                self.addSubview(canvasGroup)
            }
...
extension UIButton {
    ...
    func notChange(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        entitle(interval: interval, topOffset: 0)
    }
    ...
    func entitle(interval: CGFloat, topOffset: CGFloat) {
    ...
    func latchkeyDoingset(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func sectionDrop(color: UIColor, forState: UIControl.State) {
    ...
    func modelPriceStr(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((String(str_mePagePathData))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (String(str_mePagePathData)), with: (String(str_editName)) + "\(kLet_textSystemTitle)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.custom(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(str_groupValue.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(str_groupValue.prefix(4))), with: "_\(kLet_textSystemTitle)" + (String(str_managerName.suffix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(str_failureText))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(str_failureText)), with: "_\(kLet_textSystemTitle)" + (String(str_transactionValue.suffix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.custom(urlStr: newUrl, placeImg: nil)
    }
