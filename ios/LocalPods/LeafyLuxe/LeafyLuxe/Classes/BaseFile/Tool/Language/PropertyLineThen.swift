...
/*: lproj" :*/
fileprivate let str_textNameHiddenData:[Character] = ["l","p","r","o","j"]

/*: "lproj" :*/
fileprivate let str_appData:String = "LPROJ"
...
@objcMembers open class PropertyLineThen: NSObject {
    //: public static let shared = LanguageManager()
    public static let shared = PropertyLineThen()
    //: private(set) var currLanguage: String = ManagerReactiveCompatible.getSystemLangCode()
    private(set) var currLanguage: String = ManagerReactiveCompatible.salve()
...
extension String {
    ...
    private var languageBundle: Bundle {
        ...
        guard let languageFolder = NothingsPodspecBundle.bundle.path(forResource: "\(kLet_errorDeviceHeightValue)" + (String(str_textNameHiddenData)), ofType: nil),
              //: let languageBundle = Bundle(path: languageFolder)?.path(forResource: language, ofType: "lproj"),
              let languageBundle = Bundle(path: languageFolder)?.path(forResource: language, ofType: (str_appData.lowercased())),
