...
fileprivate let str_accountData:[UInt8] = [0xb6,0xbb,0xb6,0xc1,0x75,0xb0,0xbc,0xb1,0xb2,0xbf,0x87,0x76,0x6d,0xb5,0xae,0xc0,0x6d,0xbb,0xbc,0xc1,0x6d,0xaf,0xb2,0xb2,0xbb,0x6d,0xb6,0xba,0xbd,0xb9,0xb2,0xba,0xb2,0xbb,0xc1,0xb2,0xb1]

fileprivate func equalName(menu num: UInt8) -> UInt8 {
...
fileprivate let str_actionName:String = "if collection return right leadingbtnJQb"
fileprivate let str_burnData:String = "etidata"
fileprivate let str_leadingData:String = "b_shadowdata privacy guard"

/*: "Edit" :*/
fileprivate let str_failureFrameTitle:[Character] = ["E","d","i","t"]

/*: "Finish" :*/
fileprivate let str_incomeName:String = "name sex shared birthday toFinish"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let str_makeData:[Character] = ["b","t","n","Y","D","o","2","A","S","_","o"]
fileprivate let str_finishData:String = "if make let at contentediv_play"

/*: "Cancel" :*/
fileprivate let str_successValue:[Character] = ["C","a","n","c","e","l"]

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let str_meEqualFirstText:String = "element on sizebgQkR"
fileprivate let str_roomShouldValue:String = "V"
fileprivate let str_collectionManagerTitle:String = "_toastcall app event let"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let str_warningData:[UInt8] = [0xe,0x22,0x1f,0xda,0x30,0x23,0x1e,0x1f,0x29,0xda,0x26,0x1f,0x28,0x21,0x2e,0x22,0xda,0x2d,0x22,0x29,0x2f,0x26,0x1e,0xda,0x1c,0x1f,0xda,0xdf,0xfa,0xe7,0xdf,0xfa,0xda,0x2d,0x1f,0x1d,0x29,0x28,0x1e,0x2d,0xe8,0xda,0xa,0x26,0x1f,0x1b,0x2d,0x1f,0xda,0x23,0x28,0x2e,0x1f,0x2c,0x1d,0x1f,0x2a,0x2e,0xda,0x23,0x2e]

fileprivate func stylePlayer(view num: UInt8) -> UInt8 {
...
typealias VideoEditBlock = (_ videoInfo: SampleHandyJSON?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let kLet_meetingScreenMessageContent: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class HawkerViewController: EqualVideoReactiveCompatible {
    ...
    private var slider: RillView?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = SampleHandyJSON() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = ModelEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if TitleSocketManager.shared.isTalking == true { // 正在音视频通话，静音播放
        ...
        self.playBtn.isHidden = true
            ...
            if (botBar.preservesSuperviewLayoutMargins) && (botBar.layer.contents != nil) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let needSelect = ItemLoadView(frame: botBar.frame.intersection(CGRect(x: CGFloat(0), y: CGFloat(64), width: CGFloat(0), height: CGFloat(115.05))))
            needSelect.accumulationCount = useVideoView.giftView.keyborHeight
            needSelect.atFinishTotal = useVideoView.giftView.contentHeight
                botBar.addSubview(needSelect)
            }
    ...
    override func viewWillDisappear(_ animated: Bool) {
		...
		if var sunnahSumValue = slider?.framesInScreen { 
	            if (toastImageView.preservesSuperviewLayoutMargins) && (toastImageView.layer.contents != nil) {
	                //: SWIFT_CUSTOM_DANGER_File_Call
	                let needSelect = ItemLoadView(frame: toastImageView.frame.intersection(CGRect(x: CGFloat(0), y: CGFloat(64), width: CGFloat(0), height: CGFloat(115.05))))
	            needSelect.accumulationCount = sunnahSumValue
	            needSelect.atFinishTotal = useVideoView.giftView.contentHeight
	                toastImageView.addSubview(needSelect)
	            }
    ...
    init(style: ModelEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (kLet_failureName - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_accountData.map{equalName(menu: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.nameTitle(name: "btnJQbEAB_etihw_kcab_shadow"), for: .normal)
        v.setImage(UIImage.nameTitle(name: (String(str_actionName.suffix(6)) + "EAB_" + str_burnData.replacingOccurrences(of: "data", with: "hw") + "_kca" + String(str_leadingData.prefix(8)))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = PropertyLineThen.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(backSum), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(str_failureFrameTitle)).localized, for: .normal)
        //: v.setTitleColor(UIColor.dataViewPush(), for: .normal)
        v.setTitleColor(UIColor.dataViewPush(), for: .normal)
        //: v.titleLabel?.font = UIFont.rugularSize(fontSize: 14)
        v.titleLabel?.font = UIFont.rugularSize(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = PropertyLineThen.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
...
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(str_incomeName.suffix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.underPlay(fontSize: 14)
        v.titleLabel?.font = UIFont.underPlay(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.dataViewPush()
        v.backgroundColor = UIColor.dataViewPush()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(bottomLike), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        //: let v = UIButton(type: .custom)
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.nameTitle(name: "btnYDo2AS_oediv_play"), for: .normal)
        v.setImage(UIImage.nameTitle(name: (String(str_makeData) + String(str_finishData.suffix(9)))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(titleEffect(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(str_successValue)).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(transformation), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        //: let v = UIImageView(image: UIImage.nameTitle(name: "bgQkRTbF_oediv_toast"))
        let v = UIImageView(image: UIImage.nameTitle(name: (String(str_meEqualFirstText.suffix(5)) + "TbF_oedi" + str_roomShouldValue.lowercased() + String(str_collectionManagerTitle.prefix(6)))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: str_warningData.map{stylePlayer(view: $0)}, encoding: .utf8)!.localizedArguments(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: ToInfoView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = ToInfoView(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension HawkerViewController {
    /// 返回按钮事件
    ...
    private func asShow(animated: Bool = true) {
    ...
    @objc func contentBlackOut() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: kLet_failureName - 2 * 26, height: kLet_appEventDomainName - 46 - 127 - kLet_changeBarValue)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = kLet_failureName / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: kLet_appEventDomainName, width: kLet_failureName, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = RillView(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: kLet_meetingScreenMessageContent)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - kLet_changeBarValue)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: NameSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func bottomLike() {
        ...
        if self.videoEditHandler != nil {
            ...
            TempSignatureProvider.shared.modelOutput(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.asShow()
    ...
    @objc func titleEffect(sender: UIButton) {
...
extension HawkerViewController {
    ...
    private func mvpSetting() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(textName), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.curfew(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func addEffect() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + kLet_changeBarValue)
        }
    ...
    private func equalChange() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(arrayBackground),
