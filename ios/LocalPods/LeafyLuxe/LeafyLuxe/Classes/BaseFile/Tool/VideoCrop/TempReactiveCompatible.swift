...
fileprivate let str_endAddName:[UInt8] = [0x1d,0x1a,0x1d,0x0,0x5c,0x17,0x1b,0x10,0x11,0x6,0x4e,0x5d,0x54,0x1c,0x15,0x7,0x54,0x1a,0x1b,0x0,0x54,0x16,0x11,0x11,0x1a,0x54,0x1d,0x19,0x4,0x18,0x11,0x19,0x11,0x1a,0x0,0x11,0x10]

/*: "FFD855" :*/
fileprivate let str_blockToName:String = "FFequal5"
fileprivate let str_firstMakeValue:[Character] = ["5"]
...
class TempReactiveCompatible: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - kLet_beginTitle
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: kLet_beginTitle,
                ...
                self.buttonX(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_endAddName.map{$0^116}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (str_blockToName.replacingOccurrences(of: "equal", with: "D8") + String(str_firstMakeValue)))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: kLet_bottomAvailableName, y: 0, width: kLet_beginTitle, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - kLet_beginTitle, y: 0, width: kLet_beginTitle, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.buttonEnd(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: kLet_bottomAvailableName, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.buttonEnd(0, 0, 0, 0.6)
...
extension TempReactiveCompatible {
    ...
    func buttonX(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - kLet_beginTitle,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kLet_beginTitle)
    }
}
...
extension TempReactiveCompatible {
    ...
    @objc func addGesture(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.buttonX(startX: videoStartX)
    ...
    @objc func levelModel(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension TempReactiveCompatible {
    ...
    private func trifle() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(addGesture(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(levelModel(gesture:)))
    ...
    func curfew(_ xposition: CGFloat) {
