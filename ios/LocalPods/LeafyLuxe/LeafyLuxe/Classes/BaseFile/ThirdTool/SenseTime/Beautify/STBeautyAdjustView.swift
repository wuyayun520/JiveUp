...
class STBeautyAdjustView: UIView {
    ...
    private lazy var topItemView: STBeautyTopItemView = {
        ...
        view.frame = CGRect.init(x: 0, y: 50, width: kLet_failureName, height: 50)
        view.delegate = self
        return view
    }()
    ...
    private lazy var itemCollectionView: STBeautyCollectionView = {
        let view = STBeautyCollectionView.init(frame: CGRect.init(x: 0, y: 50+18, width: kLet_failureName, height: beautyContainerView.height-50-18-kLet_changeBarValue), collectionViewLayout: UICollectionViewLayout())
        return view
    }()
    
    ///  滤镜列表
    private lazy var filterListView: STFilterListView = {
        let view = STFilterListView.init(frame: CGRect.init(x: kLet_failureName, y: 50+18, width: kLet_failureName, height: beautyContainerView.height-50-18-kLet_changeBarValue))
        return view
    }()
    
    
    private lazy var beautySlider: STBeautySlider = {
        let view = STBeautySlider.init(frame: CGRect.init(x: 22, y: 0, width: kLet_failureName-44, height: 40))
        view.minimumTrackTintColor = UIColor.dataViewPush()
        ...
        view.setThumbImage(UIImage.nameTitle(name: "st_slider_thumb"), for: .normal)
...
extension STBeautyAdjustView: STBeautyTopItemViewDelegate {
    ...
    func show() {
        ...
        popView = ChangeShapeRoomReactiveCompatible.init(frame: UIScreen.main.bounds)
    ...
    func changeTopItem(senderTag: Int) {
        ...
        if itemType == .Filter {
            ...
            self.beautyContainerView.setContentOffset(CGPoint.init(x: kLet_failureName, y: 0), animated: true)
...
extension STBeautyAdjustView {
    ...
    func setupSubviews() {
        ...
        beautyContainerView.frame = CGRect.init(x: 0, y: 50, width: kLet_failureName, height: self.height - 50)
