...
extension STFilterLooksModel {
    ...
    func getSourceFromPath(bundleName: String, bmpType: STEffectsType) -> [STFilterLooksModel] {
        ...
        let strLocalBundlePath = NothingsPodspecBundle.bundle.path(forResource: bundleName, ofType: "bundle")
        ...
        for i in 0...(arrFiles?.count ?? 1)-1 {
            ...
            if strFileName.hasSuffix(".zip") {
                ...
                if imageThumb == nil {
                    imageThumb = UIImage.nameTitle(name: "st_none")
                }
    ...
    func getFilterBundleSource(bundleName: NSString) -> [STFilterLooksModel] {
        let arrFiles = try? FileManager.default.contentsOfDirectory(atPath: ManagerReactiveCompatible.asSub())
        ...
        for i in 0...(arrFiles?.count ?? 1)-1 {
            ...
            if strFileName.hasSuffix("model") {
                ...
                if orignName.count > 0 {
                    sourceDict[orignName] = NSString.path(withComponents: [ManagerReactiveCompatible.asSub(), strFileName ])
                }
            }
        }
